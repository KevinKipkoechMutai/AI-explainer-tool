...
export function useVapi(book: IBook) {
    ...
    const maxDurationRef = useLatestRef(limits.minutesPerSession * 60);
    ...
    const start = useCallback(async () => {
        ...
        try {
            ...
            if (!result.success) {
                setLimitError(result.message || 'Session limit reached. Please upgrade your plan.');
            ...
            if (sessionIdRef.current) {
                endVoiceSession(sessionIdRef.current, durationRef.current).catch((endErr) => {
                    console.error('Failed to end voice session:', endErr);
                })
                sessionIdRef.current = null;
            }
            setStatus('idle');
    ...
    const maxDurationSeconds = limits.minutesPerSession * SECONDS_PER_MINUTE;
